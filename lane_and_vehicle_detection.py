...
def haar_detector(img):
    ...
    classifier = cv2.CascadeClassifier('Haarcascades/cascade.xml')
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # Pass frame to our car classifier
    objects = classifier.detectMultiScale(gray, 1.4, 2)
...
while cap.isOpened():
    ...
    annotated_image = annotate_image_array(frame)
    objects = haar_detector(frame)
    ...
    for (x,y,w,h) in objects:
        ...
        cv2.imshow('lane_detection', annotated_image)
...
if __name__ == '__main__':
	...
	annotate_video('challenge', 'output_file')
